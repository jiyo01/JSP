...
<!-- jsp액션태그 중, useBean태그를 이용하여 아래와 같이 설정하였다.
	...
	class속성은 student의 원본 클래스의 패키지명.클래스명 지정을 해주어야 한다.
	이렇게 해주면 자바에서 new연산자로 객체(인스턴스)를 생성한 것과 동일한 개념이
	되는 것이다. -->
...
<body>
	...
	<!-- 여기서 중요한 것은 getProperty가 student클래스의 getter메서드를 호출하는
	 	 태그라는 것이다. -->
	...
	<!-- jsp액션태그 중 setProperty를 이용해서 name에는 bean의 id값, property는 빈의 속성
	(멤버변수), value는 실제 값을 주어 저장하는 것이다.
	...
	<!-- 여기서 중요한 것은 getProperty가 student클래스의 getter메서드를 호출하는 태그라는 것이다. -->
	이름 : <jsp:getProperty property="name" name="student"/><br>
	나이 : <jsp:getProperty property="age" name="student"/><br>
	학년 : <jsp:getProperty property="grade" name="student"/><br>
	번호 : <jsp:getProperty property="studentNum" name="student"/><br>
